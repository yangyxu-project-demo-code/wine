...
module.exports = React.createClass({
	getInitialState: function() {
    	...
    	return {
			...
			value: 0,
			type: _initType,
			data: Store.post('/hcredwine/restaurant/pagingSettlement', {
				openId: _token.openid,
				type: _initType
			})
		};
	...
	__submit: function (){
		if(this.state.value){
			Store.post('/hcredwine/merchant/withdraw', {
				openId: this.state.openId,
	...
	__onCountChange: function (event, data, index){
		...
		if(_value<this.state.info.settlementAmount){
			this.state.value = _value;
		}else {
			this.state.value = this.state.info.settlementAmount;
		}
		...
		if(_info){
			return <div className="footer">
				...
				<div className="right" onClick={this.__submit} >
					<span>申请提现</span>
					<span>额度 {_info.settlementAmount} 元</span>
				</div>
			</div>;
		}
		return null;
